...
Оголошуємо массив,потім користувач вводить розмір массиву,після цього за допомогою функцію `malloc` для виділення пам'яті,та в кінці для звільнення використаної пам'яті функцію `free`,і за допомогою циклу заповнюємо і виводимо масив.
...
Додаю нову зміну для нового розміру массиву,потім за допомогою `realloc` зменшуємо/збільшуємо пам'ять.Створюємо новий массив в який переддаємо початковий та замінюємо в ньому значення на ті що відповідають новому розміру.
```
./Ex13
Print start size of array: 3
Array:3 3 3
Print new size of array: 2
New array:2 2
```
#### Введіть обмеження на мінімальний та максимальний розмір масиву.
Додаємо за допомогю `#define` мінімальний та максимальний розмір та обмежуємо можливість користувачу вводити ці значення,шляхом закінчення програми при введені чисел  за цим діапазоном 
```
./Ex13
Print start size of array(2-6): 2
Array:2 2
Print new size of array(2-6): 7
Error!
```
#### Реалізуйте функцію перевірки використання пам’яті, яка виводить поточний стан виділеної пам’яті (malloc, realloc, free).
Використовуємо тип данних size_t для рахування байтів,створюємо ффункцію,яка буде виводити скількі байтів виділено,після цього функції для realloc(),malloc(),free(),ці функції одночасно виконують і роль цих операторів,і рахують скількі байтів виділено.
```
./Ex13
Програма керування динамічною пам'яттю
Виділено зараз:0 байтів
------------------------------------------
Print start size of array(2-6): 2
ПОДІЯ: Виділено 8 байтів за адресою 0x624d63567ac0
Виділено зараз:8 байтів
------------------------------------------
Array:2 2
Print new size of array(2-6): 6
ПОДІЯ: Зміна розміру з 8 на 24 байтів
Виділено зараз:24 байтів
------------------------------------------
New array:6 6 6 6 6 6
ПОДІЯ: Звільнено 24 байтів
Виділено зараз:0 байтів
------------------------------------------
```
